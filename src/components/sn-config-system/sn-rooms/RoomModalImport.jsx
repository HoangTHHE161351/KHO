...
const RoomModalImport = ({ isOpen, onClose }) => {
  const dispatch = useDispatch();
  const [jsonData, setJsonData] = useState([]);
  const { searchKey } = useSelector((state) => state.roomReducer);
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (e) => {
      ...
      const parsedDataRoom = XLSX.utils.sheet_to_json(worksheet, { header: 1 });
      const filteredDataClass = parsedDataRoom.filter((row) =>
  ...
  const downloadExcel = async () => {
    try {
      const url = "/excel/ImportRoom.xlsx";
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "ImportRoom.xlsx");
  ...
  const importData = async () => {
    try {
      ...
      const dataRoomToSend = jsonData.slice(1);

      // Tạo mảng đối tượng mới từ dataToSend
      const formattedDataRoom = dataRoomToSend.map((row) => ({
        roomName: row[0], // Lấy giá trị từ cột Room Name
      ...
      const response = await fetch(
        "http://localhost:8080/api/v1/room/import-room",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify(formattedDataRoom),
        }
      );
...
export default RoomModalImport;
