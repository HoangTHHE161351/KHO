...
const CurriculumModalImport = ({ isOpen, onClose }) => {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = (e) => {
      ...
      const parsedDataCurriculum = XLSX.utils.sheet_to_json(worksheet, {
        header: 1,
      });
      const filteredDataClass = parsedDataCurriculum.filter((row) =>
  ...
  const downloadExcel = async () => {
    try {
      const url = "/excel/ImportCurriculum.xlsx";
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "ImportCurriculum.xlsx");
  ...
  const importData = async () => {
    try {
      ...
      const dataCurriculumToSend = jsonData.slice(1);

      // Tạo mảng đối tượng mới từ dataToSend
      const formattedDataCurriculum = dataCurriculumToSend.map((row) => ({
        curriculumName: row[0], // Lấy giá trị từ cột curriculum Name
      ...
      const response = await fetch(
        "http://localhost:8080/api/v1/curriculum/import-curriculum",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify(formattedDataCurriculum),
        }
      );
      ...
      if (response.ok) {
        // Xử lý thành công
        dispatch(
          curriculumActions.getCurriculumList({
            search: searchKey,
...
export default CurriculumModalImport;
