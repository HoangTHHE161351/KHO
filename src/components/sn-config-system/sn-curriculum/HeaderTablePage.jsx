...
import ImportSubjectCurriculumModal from "./ImportSubjectCurriculumModal";
...
const HeaderTablePage = ({
  ...
  const [openImportSubjectCurriculum, setOpenImportSubjectCurriculum] =
    useState(false);
  ...
  const onExportExcel = async () => {
    try {
      const columns = [
        ...
        { name: "Name", key: "curriculumName", width: 30 },
  ...
  return (
    <Grid container columnSpacing={2}>
      ...
      <Grid flex={1} container justifyContent="flex-end">
        <Stack direction={"row"} spacing={1}>
          <Button
            variant="contained"
            color="success"
            onClick={() => setOpenImportSubjectCurriculum(true)}
          >
            Import Subject Curriculum Excel
          </Button>
          <Button
            variant="contained"
            color="success"
      ...
      <ImportModal
        ...
        onSuccess={onSuccess}
      />
      <ImportSubjectCurriculumModal
        open={openImportSubjectCurriculum}
        onClose={() => setOpenImportSubjectCurriculum(false)}
        onSuccess={onSuccess}
      />
    </Grid>
  );
