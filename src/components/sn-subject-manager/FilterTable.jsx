...
import SubjectManageAddModal from "./SubjectManagerAddModal";
...
const FilterTable = () => {
  ...
  const { filter } = useSelector((state) => state.subjectReducer);

  const handleChangeFilterWithKey = useCallback(
    (key) => (value) => {
      dispatch(
        subjectActions.changeFilterWithKey({
          key,
          value,
        })
      );
  ...
  return (
    <>
      <Grid container columnSpacing={2}>
        ...
        {pathname === PathConstants.SUBJECT_MANAGER && (
          <Grid item container flex={1} justifyContent={"flex-end"}>
            <Stack direction={"row"} spacing={1}>
              <Button
                ...
                color="success"
                size="small"
                onClick={() => handleExportExcel(filter, "subject_export")}
              >
                Export Excel
              </Button>
              <Button
      ...
      <SubjectManageAddModal open={open} onClose={() => setOpen(false)} />
    </>
  );
};
